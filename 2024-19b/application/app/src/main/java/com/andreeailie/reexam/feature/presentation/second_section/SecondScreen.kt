            ...
            when (networkStatus) {
                NetworkStatusTracker.NetworkStatus.Available -> {
                    Column (
                        ...
                        PropertyList(viewModel = viewModel, navController = navController, topEntities = topEntities)
...
fun EntityItem(entity: Entity) {
    Row(
        ...
        Column(
            ...
            Text(
                text = "Year: " + entity.year.toString(),
                style = MaterialTheme.typography.headlineSmall,
            )
        }
    }
